...
class FirstOrderDiscreteTimeSystem(DiscreteTimeSystem):
    ...
    @property
    def u_lo(self):
        return self.continuous_time_system.u_lo

    @property
    def u_up(self):
        return self.continuous_time_system.u_up
...
class SecondOrderDiscreteTimeSystem(DiscreteTimeSystem):
    ...
    def forward(self, x, u):
        ...
        if self.nq == 0:
            dx = self.continuous_time_system.forward(x, u) * self.dt
        else:
            ...
            if self.position_integration == IntegrationMethod.MidPoint:
                qdot_next = x[:, self.nq :] + dqdot
                dq = (qdot_next + x[:, self.nq :]) / 2 * self.dt
            elif self.position_integration == IntegrationMethod.ExplicitEuler:
                dq = x[:, self.nq :] * self.dt
            else:
                raise NotImplementedError
            dx = torch.concat([dq, dqdot], dim=-1)
